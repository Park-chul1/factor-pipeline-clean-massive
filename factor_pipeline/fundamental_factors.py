...
def flatten_financial_record(row: pd.Series | dict, lag_days: int = 60) -> dict:
    ...
    out = {
        "ticker": (r.get("tickers") or [r.get("ticker")])[0] if isinstance(r.get("tickers"), list) else r.get("ticker"),
    ...
    out.update({
        ...
        "capex": _deep_get_value(fin, CF, ["capital_expenditure", "payments_to_acquire_property_plant_and_equipment"]),
        "free_cash_flow": np.nan,
    })
    if np.isfinite(out["operating_cash_flow"]):
        capex = 0.0 if not np.isfinite(out["capex"]) else out["capex"]
...
def fundamentals_to_daily(flat: pd.DataFrame, dates: pd.DatetimeIndex, tickers: list[str]) -> dict[str, pd.DataFrame]:
    ...
    value_cols = [c for c in flat.columns if c not in {"ticker", "start_date", "end_date", "filing_date", "available_date", "availability_source", "timeframe", "fiscal_period", "fiscal_year"}]
